...
- Profondità: `PointerDepth` scrive `--pointer-x/y` e `--scroll-depth` su `:root` solo su desktop con puntatore fine. Su compact/touch il compositor non aggiorna i piani 3D; il CSS spegne prospettiva e derive (`@media (max-width: 63.99rem), (hover: none) and (pointer: coarse)`). Lo svelamento dell'immagine sulla mappa resta.
