...
| 2026-08-20 | **Scroll della mappa senza scatti.** La `transition: transform 700ms` su elementi che leggevano `--scroll-depth` / `--pointer-*` aggiornati a ogni frame faceva ripartire l'interpolazione CSS e produceva lo stutter. Lo smorzamento del puntatore è in JS; lo scroll è 1:1 col contenuto; durante lo scroll `html.is-scrolling` mette in pausa le derive. Atmosfera con opacità, non `filter:blur` su piani in movimento. |
| 2026-08-16 | **Profondità dello sfondo** con `perspective` + `translateZ`: piani a distanze diverse, tunnel di telai verso il punto di fuga, sbiadimento atmosferico, parallasse da puntatore e scroll. Un solo listener (`PointerDepth`) scrive le variabili su `:root`; tutto il resto è CSS. Coerente con docs/06 («profondità 2.5D leggera tramite transform e prospettiva CSS», niente scena 3D). |
